...
def helper(dis):
    ...
    desc = ' '.join(desc_series.dropna())

    pre_series = precautions[precautions['Disease'] == dis][
        ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    pre = [item for sublist in pre_series.values.tolist() for item in sublist if pd.notna(item)]

    med_series = medications[medications['Disease'] == dis]['Medication']
    med = [med_item for med_item in med_series.dropna()]

    die_series = diets[diets['Disease'] == dis]['Diet']
    die = [die_item for die_item in die_series.dropna()]

    wrkout_series = workout[workout['disease'] == dis]['workout']
    wrkout = [wrkout_item for wrkout_item in wrkout_series.dropna()]
...
diseases_list = {
    ...
    34: 'Pneumonia', 13: 'Dimorphic hemorrhoids (piles)', 18: 'Heart attack',
    39: 'Varicose veins', 26: 'Hypothyroidism', 24: 'Hyperthyroidism', 25: 'Hypoglycemia',
    31: 'Osteoarthritis', 5: 'Arthritis', 0: 'Vertigo', 2: 'Acne',
